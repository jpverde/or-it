...
impl DBMongo {
    pub fn init() -> Self {
        ...
        let db = client.database("Or-it-Cluster");
        DBMongo { db }
    }
    ...
    pub fn create_owner(&self, new_owner: Owner) -> Result<Owner, Error> {
        ...
        let data = col.insert_one(new_doc, None).expect("Error creating owner");
        let new_owner = Owner {
    ...
    pub fn get_owners(&self) -> Result<Vec<Owner>, Error> {
        let col = DBMongo::col_helper::<Owner>(self, "owner");
        let cursors = col.find(None, None).expect("Error getting list of owners");
    ...
    pub fn get_projects(&self) -> Result<Vec<Project>, Error> {
        let col = DBMongo::col_helper::<Project>(self, "project");
        let cursors = col.find(None, None).expect("Error getting lit of projects");
